...
if(message.author.id !== "842005344027672576") if(message.author.id !== "994246863903531151") return message.channel.send("hoop bilader sahip komutu bu");
  ...
  const bakiye = await db.fetch(`coinbakiye_${silinecekkllnc.id}`);
  const hesapdurumu = await db.fetch(`coinhesapdurum_${silinecekkllnc.id}`);
  const hesapismi = await db.fetch(`coinhesapismi_${silinecekkllnc.id}`);
  
  if(!hesapdurumu) return message.channel.send(`Kayıtlı olan bir kullanıcı belirtmelisin!`)
  await db.add(`coinbakiye_${silinecekkllnc.id}`, para)
