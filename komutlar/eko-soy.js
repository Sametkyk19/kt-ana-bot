...
\`\`\`Bilgi;\`\`\`
**⭐ Ak47 ile Banka soyarsınız almak için (a?blackmarket ak-47)
...
`
  ...
  if(db.has(`${user.id}.kalkan`) === false) {
    ...
    db.set(`para_${user.id}`, - random);

    dbs.set(`soygun_${message.author.id}`, Date.now());

    db.set(`${message.author.id}.bıçak_adet`, bıçak - 1)
    
    db.set (`para_${message.author.id}`, + random)
    
    db.set (`para_${message.author.id}`, + random)
    
    db.set (`para_${message.mentions.users.first()}`, para - random)
    let embed = new Discord.MessageEmbed()
      .setDescription(
