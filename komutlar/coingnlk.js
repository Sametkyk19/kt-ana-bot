...
// -------------------------------------------------------------------  
  ...
  let timeout = 86400000 //bunu ellemeyin 24 saat 
    ...
    let daily = await db.fetch(`günlükcoin_${message.author.id}`);
  ...
  ///////////////////////////////////////
    ...
    if (daily !== null && timeout - (Date.now() - daily) > 0) {
        ...
        message.channel.send(`Günlük Coin'ini tekrar alabilmek için **${time}** ***saat*** beklemelisin!`)
   
    .catch();
    } else {
      if(client.ekoayarlar.dbloy == false) {
        db.set(`günlükcoin_${message.author.id}`, Date.now())
        if(client.ekoayarlar.rastgelecoin == true) {
          const randomizer = getRandomInt(client.ekoayarlar.mincoin, client.ekoayarlar.maxcoin)
          db.add(`coinbakiye_${message.author.id}`, randomizer)
          let para1 = new Discord.MessageEmbed()
          .setColor(client.ekoayarlar.renk)
          .setDescription(`**Günlük Coin**`)
          .addField(`Aldığınız Miktar;`, randomizer + ` ${client.ekoayarlar.parabirimi}`)
          message.channel.send(para1)
        } else {
            if(client.ekoayarlar.rastgelecoin == false) {
                db.add(`coinbakiye_${message.author.id}`, client.ekoayarlar.günlükcoin)
            let para1 = new Discord.MessageEmbed()
            .setColor(client.ekoayarlar.renk)
            .setDescription(`**Günlük Coin**`)
            .addField(`Aldığınız Miktar;`, client.ekoayarlar.günlükcoin + ` ${client.ekoayarlar.parabirimi}`)
            message.channel.send(para1)
          }
        }
      } else {
          ...
          dbl.hasVoted(message.author.id).then(voted => {
            if(voted) {
              ...
              if(client.ekoayarlar.rastgelecoin == true) {
                ...
                .setColor(client.ekoayarlar.renk)
                .setDescription(`**Günlük Coin**`)
                .addField(`Aldığınız Miktar;`, randomizer + ` Coin`)
                message.channel.send(para1)
              } else {
                if(client.ekoayarlar.rastgelecoin == false) {
                  db.add(`coinbakiye_${message.author.id}`, client.ekoayarlar.günlükcoin)
